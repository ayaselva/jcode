...
#[test]
fn test_openai_service_tier_defaults_to_auto() {
    assert_eq!(
...
fn test_generated_default_config_uses_low_openai_reasoning_effort() {
    ...
    assert!(
        content.contains("openai_service_tier = \"auto\""),
        "generated default config should keep OpenAI fast mode off by default"
    );
    assert!(
